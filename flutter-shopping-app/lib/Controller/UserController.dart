...
class UserController {
  ...
  Future<ResponseLogin> editProfile(String name,String id) async {
    final token = await secureStorage.readToken();

    final response = await http.put(Uri.parse('${URLS.URL_API}/users/$id'),
  ...
  Future<ResponseDefault> registerClient(
      String name, String email, String password, String nToken) async {
    final resp = await http.post(Uri.parse('${URLS.URL_API}/users'), headers: {
  ...
  Future<List<ListAddress>?> getAddresses() async {
    ...
    final response = await http
        .get(Uri.parse('${URLS.URL_API}/users/get-addresses'), headers: {
      'Accept': 'application/json',
  ...
  Future<ResponseDefault> addNewAddressLocation(
      String street, String reference) async {
    final token = await secureStorage.readToken();

    final resp =
        await http.put(Uri.parse('${URLS.URL_API}/users/profile'), headers: {
      'Accept': 'application/json',
      'Authorization': 'Bearer $token'
    }, body: {
      'address': {
        {'street': street, 'reference': reference}
      }
    });
  ...
  Future<ResponseDefault> updateNotificationToken() async {
    ...
    final resp = await http.put(
        Uri.parse('${URLS.URL_API}/users/update-notification-token'),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer ' + token!
        },
